...
from src.adapters.database.postgres import posts
...
@pytest.fixture
def dict_post() -> dict:
    return {'author_name': 'aaa', 'title': 'bbb', 'body': 'ccc'}
...
class TestPosts:
    ...
    def test_post_create(self, client: FlaskClient, dict_post: dict):
        response = client.post('/api/posts', json=dict_post)
        assert response.json['authorName'] == dict_post["author_name"]
        assert response.json['title'] == dict_post["title"]
        assert response.json['body'] == dict_post["body"]
